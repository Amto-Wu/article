...
所有的预处理器命令都是以（#）开头。它必须是第一个非空字符，为了增强可读性，预处理器指令应从第一列开始。下面列出了所有重要的预处理器指令：
...
ANSI C 定义了许多宏。在编程中可以使用这些宏，但是**不能直接修改这些预定义的宏**。
...
```c
File :C:\Users\qingtian\Desktop\未命名1.cpp
Date :Apr  6 2022
Time :18:33:39
Line :6
ANSI :1
...
宏定义内的标记粘贴运算符（##）会合并两个参数。它允许在宏定义中两个独立的标记被合并为一个标记，**其实是一个标识符**。例如：
...
C程序的一个强大的功能是可以使用参数化的宏来模拟函数。例如，下面的代码是计算一个数的平方：
