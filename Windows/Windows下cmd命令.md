...
解释：表示系统会在 60 秒后关机，可用 shutdown -a 解除
...
ping -l 2000 -t 192.168.1.1
# 指定每次以2000bytes不间断常ping指定的主机
```

``` basic
tracert		跟踪路由，常用于网络连通性的排查
```
...
``` basic
sc		SC 是用来与服务控制管理器和服务进行通信的命令行程序
```
